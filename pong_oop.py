...
"TODO:fullscreen, закручивание мяча, линия "
# Зачем?
...
class Ball:
    def __init__(self) -> None:  # Как сделать большой жёлтый мячик в правом углу?
        ...
        self.speed_x, self.speed_y = degrees_to_velocity(randint(45, 136), 5)
        self.color = (255, 255, 255)
        self.rect = None
        self.iscollided = False

    def rotate_ball(self):
        """изменяет ускорение мяча они могут быть от 45-135 или 225-315"""
        if randint(0, 1):
            self.x, self.y = degrees_to_velocity(randint(45, 136), 10)
        else:
            self.x, self.y = degrees_to_velocity(randint(225, 315), 10)
...
def racket_collisions(ball: Ball, rackets: list) -> None:
    ...
    """
    if ball.rect.colliderect(rackets[0].rect) and not ball.iscollided or ball.rect.colliderect(rackets[1].rect) and not ball.iscollided:  # О-о-о-очень длинно и непонятно. Is - плохая проверка для bool
